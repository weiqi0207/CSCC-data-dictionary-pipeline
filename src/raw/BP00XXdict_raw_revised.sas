...
%macro run_it(ds=,cl=,vl=);
	...
	data merged_five;
        retain VARNUM VARNAME VARDESC DOCFILE TYPE UNITS RESOLUTION COMMENT1 COMMENT2 QC_COMMENTS VARIABLE_SOURCE SOURCE_VARIABLE_ID VARIABLE_MAPPING UNIQUEKEY COLLINTERVAL VALUES N MISS_N MISS_PERCENT DISTINCT_N MIN MAX MEAN STD PCT25 Median PCT75;
        set merged_five;
    run;
	...
	/* Reclassify date variable as date type */
	/*data metadataset1_sorted;
	    set metadataset1_sorted;
	...
	/* Export the xslv file */
    proc export data=metadataset1_sorted
	    outfile="&outdir.&ds..xlsx"
	    dbms=xlsx
	    replace;
	run;
